...
namespace net {
	...
	bool udp_socket::open(uint32_t port)
	{
	...
	bool udp_socket::send(net::address const & dest, void const * data, uint32_t packet_size)
	{
		sockaddr_in addr;
		addr.sin_family = AF_INET;
	...
	int udp_socket::receive(net::address & sender, void * data, uint32_t size)
	{
		//uint8_t packet_data[256];
		uint32_t maximum_packet_size = 512;
		...
		int received_bytes = recvfrom(handle, (char*)data, maximum_packet_size,
